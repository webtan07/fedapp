// Vercel Build Output API function entry — mirrors instagram-automation's.
//
...
// fetch handler (dist/server/server.js), so we adapt: Node IncomingMessage →
// web Request, run the fetch handler, stream the web Response onto
// ServerResponse. Node 22 has global Request/Response/Headers/ReadableStream.
//
...
export default async function vercelHandler(
  ...
  try {
    ...
    // return a stack trace to the public visitor.
    console.error("[FED] SSR request failed", error);
    res.statusCode = 500;
