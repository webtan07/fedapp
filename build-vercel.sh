#!/usr/bin/env bash
# Produce a Vercel Build Output API bundle (.vercel/output) for FED, then
# deploy with:  bunx vercel deploy --prebuilt
#
# Adapted from instagram-automation/build-vercel.sh (the proven pipeline):
...
#    real files via --splitting) in a single render.func removes all Vercel
...
# runtime ("Server function module export not resolved"), breaking every
